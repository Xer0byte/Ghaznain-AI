...
export async function generateImage(prompt: string, aspectRatio: string = "1:1", base64Image?: string, mimeType?: string) {
    try {
      const ai = getAiClient();
      const enhancedPrompt = `Photorealistic, raw photo, highly detailed, 8k resolution, natural lighting, shot on 35mm lens, realistic textures, lifelike, unedited photographic photography style. Essential subject: ${prompt}`;
